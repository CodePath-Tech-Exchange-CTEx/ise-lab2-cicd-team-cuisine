...
def test_login_creates_user_when_account_not_found(monkeypatch):
    monkeypatch.setattr(st, 'radio', lambda *args, **kwargs: 'Log in')
    ...
    def fake_create_user(username, full_name, date_of_birth):
        created['called'] = True
        assert username == 'new_user'
        assert full_name == 'New_User'
        assert date_of_birth == '2000-01-01'
        return 'new_user_id'

    monkeypatch.setattr(home, 'create_user', fake_create_user)
    ...
    assert result is False
    assert st.session_state['logged_in'] is True
    assert st.session_state['username'] == 'new_user_id'
